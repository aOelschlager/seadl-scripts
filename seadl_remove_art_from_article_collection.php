...
$csv = array_map('str_getcsv', file('/file/path/art_articles_pids.csv'));
foreach ($csv as $row) {
	$id = $row[0];
	print "Working on PID: " . $id . "\n";
	$object = $repository->getObject($id);
	$object->relationships->remove('info:fedora/fedora-system:def/relations-external#', 'isMemberOfCollection','SEAImages:VNArtNews');
